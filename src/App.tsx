...
export default function App() {
  return (
    <Router>
      <Routes>
        <Route path="/" element={<Navigate to={isEnvValid() === true ? '/home' : '/error'} replace />} />
...
const isEnvValid = () => {
  ...
  const reducer = (previousValue: boolean, currentValue: string) => previousValue && Boolean(process.env[currentValue]);
...
const Home = () => {
  ...
  if (engine) {
    return <div className="App">{engine && <HomePage engine={engine} />}</div>;
  } else {
...
const Search = () => {
  ...
  if (engine) {
    return <div className="App">{engine && <SearchPage engine={engine} />}</div>;
  } else {
    return <div>Waiting for engine</div>;
  }
};
        ...
        <Grid item md={9} sm={11}>
          <div className="container">
            ...
            <Typography variant="body1">
              You should have a valid <code>.env</code> file at the root of this project. You can use <code>.env.example</code> as starting
              point and make sure to replace all placeholder variables
              <code>&#60;...&#62;</code> by the proper information for your organization.
            </Typography>
            <p>
